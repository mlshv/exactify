...
if (!fs.existsSync('package.json')) {
    log('exactify is a CLI tool that replaces all inexact package.json versions with specific versions from package-lock.json');
    log(example);
...
log('🙌 You are going to replace all inexact package.json versions with specific versions from package-lock.json');
log();
log();
log(example);
log();
...
const isSaveExactSet = fs.existsSync('.npmrc') &&
    fs.readFileSync('.npmrc').toString().includes('save-exact=true');
if (!isSaveExactSet) {
    prompts.push({
        type: 'confirm',
        name: 'isSaveExactSetConfirmed',
        message: `Do you also want to add save-exact=true in your .npmrc? ${chalk.dim('(recommended)')}`,
...
inquirer
    ...
    if (isConfirmed) {
        ...
        const pacakgeLockJson = JSON.parse(packageLockJsonContent);
        ...
        const vesrionsFromPackageLock = Object.fromEntries(Object.entries(pacakgeLockJson.packages)
        ...
        const packagesToUpdate = Object.entries(depsWithCarets).map(([packageName, inexactVersion]) => {
            var _a;
            const realVersion = (_a = vesrionsFromPackageLock[packageName]) !== null && _a !== void 0 ? _a : inexactVersion.replace('^', '');
        ...
        log(`${Object.keys(packagesWithUpdatedMinorVersion).length} minor versions were updated with actual versions from package-lock.json`);
        log();
